...
utf8_bytes(text)  # 25 bytes

# Detailed codepoint information
for cp in codepoints(text):
    print(f"{cp.index}: '{cp.char}' U+{cp.codepoint} ({cp.name})")
# 0: 'H' U+0048 (LATIN CAPITAL LETTER H)
# 1: 'e' U+0065 (LATIN SMALL LETTER E)
# ...
# 9: '世' U+4E16 (CJK UNIFIED IDEOGRAPH-4E16)
# 10: '界' U+754C (CJK UNIFIED IDEOGRAPH-754C)
# 13: '🌍' U+1F30D (EMOJI GLOBE WITH MERIDIANS)
...
# Returns list of InvisibleCharInfo for each invisible character

# Safe representation that shows invisible characters
visible_repr("hello\u200Bworld")
# 'hello\x02ZWSP\x03world' (ZWSP rendered as marker)
```
...
#  DiffSpan(kind='replace', a_span=(1,2), b_span=(1,2), a_text='b', b_text='x'),
#  DiffSpan(kind='equal', a_span=(2,3), b_span=(2,4), a_text='c', b_text='bc')]
```
...
# CheckBracketsResult(balanced=True, unmatched_openers=[], unmatched_closers=[])

# Unbalanced - returns details about mismatches
result = check_brackets("(a + b]")  # Round vs square mismatch
# CheckBracketsResult(
#   balanced=False,
#   unmatched_openers=[BracketError(char='(', index=0, line=1, column=1)],
#   unmatched_closers=[BracketError(char=']', index=5, line=1, column=6)]
# )
...
#   valid_pattern=True,
#   results=[
#     RegexMatch(sample='123-4567', matches=True, fullmatch=False, span=(0,8),
#               groups=('123','4567'), groupdict={}),
#     RegexMatch(sample='hello', matches=False, ...),
#     RegexMatch(sample='555-1234', matches=True, ...)
#   ]
# )

# Invalid regex
result = regex_test(r"[invalid", ["test"])
# RegexTestResult(valid_pattern=False, results=[])
```
...
# TextEqualResult(equal=True, ...)

# When not equal
result = text_equal("hello", "world")
# TextEqualResult(
#   equal=False,
#   classification='completely_different',
#   first_difference=FirstDiff(a_index=0, ...),
#   ...
# )
```

**Comparison modes:** `raw` (byte identity), `NFC`, `NFD`, `NFKC`, `NFKD`, `casefold`, `trim`
...
# ExplainDiffResult(
#   equal=False,
#   classification='confusable_characters',
#   summary='Strings contain confusable Unicode characters',
#   diffs=[
#     DiffInfo(
#       kind='replace',
#       a_span=(1,2),
...
#       a_text='а',
#       b_text='a',
#       a_codepoints=[('а','U+0430')],
#       b_codepoints=[('a','U+0041')],
#       note='CYRILLIC SMALL LETTER A looks like LATIN SMALL LETTER A'
#     )
#   ],
#   security_findings=[
#     'Character at index 1 is confusable (U+0430 Cyrillic vs U+0041 Latin)'
#   ],
#   agent_instruction='Both strings appear similar but contain confusable Unicode...'
# )
```
...
# InspectTextResult(
#   safe_repr='user\x02ZWSP\x03name',
#   metrics={...},
#   normalization={...},
#   invisibles=[
#     InvisibleCharInfo(
...
#   confusables=[],
#   warnings=[
#     'Text contains invisible character ZERO WIDTH SPACE at index 4',
#     'Invisible characters may indicate hidden content or spoofing attempts'
#   ]
# )
```
...
# {
#   'same_ordered': [],
#   'same_unordered': ['apple', 'cherry'],  # Case differs but same after casefold
#   'only_in_a': ['banana'],
#   'only_in_b': ['date'],
#   'duplicates': {},
#   'near_matches': []
...
# With ignore_order=False (must be in same position)
result = list_compare(
    ["a", "b", "c"],
    ["c", "b", "a"],
    ignore_order=False
)
# {'same_ordered': [], 'only_in_a': ['a', 'b', 'c'], 'only_in_b': ['c', 'b', 'a'], ...}
```
...
def validate_safe_text(text: str) -> tuple[bool, list[str]]:
    ...
    if result.normalization.mixed_scripts:
        warnings.append(f"Mixed Unicode scripts detected")
...
#  "Character at index 1 is confusable (Cyrillic 'а' looks like Latin 'a')"]
```
